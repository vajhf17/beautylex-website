...
function Navbar() {
  ...
  return (
<nav
  className={"fixed top-0 left-0 w-full z-50 bg-brand-light shadow-md"}
>
  <div className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-10 py-4">
      ...
      {/* AQUÍ VA EL BLOQUE DE SERVICIOS */}
        <li className="relative group">
          <Link to="/servicios"
          className="hover:text-brand-gold transition flex items-center gap-1 uppercase">
          Servicios
          ...
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-80 bg-white text-brand-blue rounded-2xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden">
            ...
            <Link to="/servicios/juridicos"
            className="block px-6 py-4 hover:bg-gray-50 border-b border-gray-100">
            Servicios Jurídicos
            </Link>

            <Link to="/servicios/otros"
            className="block px-6 py-4 hover:bg-gray-50">
            Otros Servicios
            </Link>

          </div>
      {/* FINAL BLOQUE DE SERVICIOS */}
        ...
        <li>
          <Link
            to="/contacto"
            className="hidden md:inline-flex bg-brand-gold text-white px-5 py-2 rounded-full text-sm font-medium hover:opacity-90 transition"
          >
            Contacto
          </Link>
        </li>
